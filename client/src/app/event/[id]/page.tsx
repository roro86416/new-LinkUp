...
} from '../../../components/card/HomeEventCard';
...
const Breadcrumb = ({ title }: { title: string }) => (
	<nav className="inline-flex items-center gap-2 text-sm text-gray-200 mb-6 animate-in fade-in slide-in-from-left-2 duration-500 bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 shadow-lg">
		<Link
			href="/"
			className="hover:text-[#EF9D11] transition-colors flex items-center gap-1"
		>
			<Home size={14} /> 首頁
		</Link>
		...
		<ChevronRight size={14} className="text-gray-400" />
		<span
			className="text-white font-medium truncate max-w-[150px] md:max-w-xs"
			title={title}
		>
			{title}
		</span>
			...
			<div className="flex flex-col md:flex-row items-center gap-6 relative z-10">
				<div className="text-center md:text-left min-w-[160px]">
					<h4 className="font-bold text-xl mb-1 flex items-center justify-center md:justify-start gap-2">
						...
						<span
							className={`text-xs font-normal ${badgeColor} px-2 py-0.5 rounded text-white`}
						>
							{badgeText}
						</span>
					</h4>
					...
					<p className="text-xs text-gray-400 mt-1">
						{timeLabel}：
						{new Date(weatherData.obsTime).toLocaleString('zh-TW', {
							month: 'numeric',
							day: 'numeric',
							hour: '2-digit',
							minute: '2-digit',
				...
				<div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm flex-1 w-full md:w-auto">
					...
					<div className="flex items-center gap-3">
						<div className="p-2 bg-white/10 rounded-lg">
							<Thermometer size={20} className="text-red-300" />
						</div>
						<div>
				...
				<div className="text-center min-w-[120px]">
					...
					<div className="flex text-[#EF9D11] text-sm justify-center my-1">
						{[...Array(5)].map((_, i) => (
							<StarSolid
								key={i}
								className={`w-4 h-4 ${i < Math.round(average || 0) ? 'text-[#EF9D11]' : 'text-gray-600'}`}
							/>
						))}
					</div>
				...
				<div className="flex-1 w-full">
					{isWriting ? (
						...
						<div className="bg-[#EF9D11]/10 border border-[#EF9D11]/30 rounded-xl p-4 relative animate-in fade-in group">
							<div className="absolute -top-3 -left-2 bg-[#EF9D11] text-white text-[10px] px-2 py-0.5 rounded shadow-sm">
								您的評價
							</div>
							...
							<div className="flex items-center gap-2 mb-2">
								...
								<div className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center text-white font-bold overflow-hidden border border-white/20">
									{myReview.user_avatar ? (
										<img
											src={myReview.user_avatar}
											alt=""
											className="w-full h-full object-cover"
										/>
									) : (
										<span className="text-sm">
											{myReview.user_name?.[0] || 'U'}
										</span>
									)}
								</div>
								...
								<div className="flex gap-0.5">
									{[...Array(5)].map((_, i) => (
										<StarSolid
											key={i}
											className={`w-4 h-4 ${i < myReview.rating ? 'text-[#EF9D11]' : 'text-gray-600'}`}
										/>
									))}
								</div>
							</div>
			...
			<div className="space-y-6">
				{otherReviews && otherReviews.length > 0 ? (
					otherReviews.map(review => (
						<div
							key={review.id}
							className="border-b border-white/10 pb-6 last:border-0 animate-in fade-in slide-in-from-bottom-2 duration-500"
						>
							<div className="flex items-center justify-between mb-2">
								...
								<div className="flex gap-0.5">
									{[...Array(5)].map((_, i) => (
										<StarSolid
											key={i}
											className={`w-4 h-4 ${i < review.rating ? 'text-[#EF9D11]' : 'text-gray-700'}`}
										/>
									))}
								</div>
							</div>
...
export default function EventDetailPage() {
	...
	const [activeTab, setActiveTab] = useState<
		'info' | 'ticket' | 'organizer' | 'map' | 'weather' | 'rating'
	>('info');
	...
	const handleSubmitReview = async (rating: number, comment: string) => {
		...
		const myReview = event.reviews?.find((r: any) => r.user_id === user.userId);
	...
	if (loading) {
		return (
			<div className="min-h-screen flex items-center justify-center bg-[#0C2838] text-white">
				<Sparkles className="animate-spin w-8 h-8 text-[#EF9D11]" />
			</div>
		);
	}
			...
			<section className="relative w-full h-[700px] md:h-[740px] overflow-hidden">
				...
				>
					<Image
						...
						alt={event.title}
						fill
						className="object-cover object-top blur-sm scale-105 opacity-60"
					/>
				...
				<div className="relative z-10 container mx-auto px-4 h-full flex flex-col justify-end pb-12 md:pb-20">
					...
					<div className="flex flex-col md:flex-row gap-8 items-end">
						...
						<div className="flex-1 text-white mb-4 md:mb-0">
							...
							{event.subtitle && (
								<h2 className="text-xl text-gray-200 mb-4 font-medium drop-shadow-md">
									{event.subtitle}
								</h2>
							)}
							...
							<div className="flex flex-wrap gap-4 text-sm md:text-base text-gray-100 mt-6 font-medium drop-shadow-md">
								<div className="flex items-center gap-2 bg-black/30 px-3 py-1.5 rounded-lg backdrop-blur-sm border border-white/10">
									<Calendar size={18} className="text-[#EF9D11]" />{' '}
									{formatDate(event.start_time)}
								</div>
								<div className="flex items-center gap-2 bg-black/30 px-3 py-1.5 rounded-lg backdrop-blur-sm border border-white/10">
									<MapPin size={18} className="text-[#EF9D11]" />{' '}
									{event.location_name}
								</div>
							</div>
						</div>
			...
			<div className="relative z-20 container mx-auto px-4 -mt-4 md:-mt-2">
				<div className="flex flex-col lg:flex-row gap-8 items-start">
					...
					<div className="flex-1 min-w-0 w-full">
						<div
							ref={tabsRef}
							className="bg-[#0f172a]/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl flex flex-col min-h-[500px]"
						>
							<div className="flex gap-3 border-b border-white/10 pb-4 mb-6 overflow-x-auto scrollbar-hide flex-shrink-0">
								{[
									...
									<button
										...
										className={`flex items-center gap-2 px-5 py-2.5 rounded-full transition-all whitespace-nowrap font-bold text-sm ${activeTab === tab.id ? 'bg-[#EF9D11] text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
									>
							...
							<div className="text-gray-200 leading-relaxed animate-in fade-in slide-in-from-bottom-2 duration-300 flex-1 flex flex-col">
									...
									<div className="space-y-4">
										{event.ticketTypes?.map((t: any) => (
											...
											>
												...
												<div className="text-[#EF9D11] font-bold text-2xl">
													NT$ {t.price}
												</div>
											</div>
										))}
								...
								{activeTab === 'map' && (
									<div className="space-y-4">
										<h3 className="font-bold text-white flex gap-2">
											<MapPin className="text-[#EF9D11]" /> 活動地點
										</h3>
								...
								{activeTab === 'weather' && (
									<EventWeather
										...
										location={event.location_name.slice(0, 3)}
									/>
								)}
										...
										<div className="flex flex-col md:flex-row items-center md:items-start gap-8">
											...
											<div className="text-center md:text-left">
												...
												<p className="text-gray-400 mb-4 max-w-lg">
													致力於舉辦最優質的活動體驗，歡迎追蹤我們的主辦專頁以獲得最新活動資訊。
												</p>
										...
										<div className="border-t border-white/10 pt-6">
											...
											<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
												{relatedEvents.slice(0, 2).map(e => (
													<div
														key={e.id}
														className="flex gap-3 bg-white/5 p-3 rounded-lg hover:bg-white/10 transition-colors cursor-pointer border border-white/5"
													>
					...
					<div className="lg:w-80 flex-shrink-0 mt-8 lg:mt-0 sticky top-24 h-fit self-start">
						<div className="bg-[#0f172a]/80 backdrop-blur-xl border border-white/20 rounded-3xl p-6 shadow-xl ring-1 ring-white/10">
							<h3 className="text-xl font-bold text-white mb-5 pb-3 border-b border-white/10">
								活動資訊
							</h3>
							<div className="space-y-5 text-sm text-gray-300">
								...
								<div className="flex items-start gap-3">
									<div className="p-2 rounded-lg bg-white/5">
										<Clock size={18} className="text-[#EF9D11]" />
									</div>
									<div>
								...
								<div className="flex items-start gap-3">
									<div className="p-2 rounded-lg bg-white/5">
										<DollarSign size={18} className="text-[#EF9D11]" />
									</div>
									<div>
										<span className="block font-bold text-white">票價</span>
										<span>詳見票種資訊</span>
									</div>
								</div>
							</div>
							<div className="mt-8 space-y-3">
								<button
									...
									className="w-full bg-gradient-to-r from-[#EF9D11] to-[#d88d0e] hover:from-[#FFAF2E] hover:to-[#EF9D11] text-white font-bold py-3.5 px-4 rounded-xl shadow-lg transition-all transform hover:-translate-y-0.5 active:scale-95 flex items-center justify-center gap-2"
								>
									<Ticket size={20} /> 立即購票
								</button>
								<div className="flex gap-2">
									<button
										...
										className={`flex-1 py-2.5 rounded-xl border font-medium text-sm transition-all flex items-center justify-center gap-1.5 ${favorited ? 'bg-red-500/10 border-red-500 text-red-500' : 'border-white/10 bg-white/5 text-gray-300 hover:bg-white/10'}`}
									>
										<Heart
											size={16}
									...
									<button
										onClick={handleShare}
										className="flex-1 py-2.5 rounded-xl border border-white/10 bg-white/5 text-gray-300 hover:bg-white/10 font-medium text-sm transition-all flex items-center justify-center gap-1.5"
									>
										<Share2 size={16} /> 分享
									</button>
								</div>
						...
						<div className="bg-[#0C2838]/80 backdrop-blur-sm border border-white/10 rounded-2xl p-4 mt-4">
							<p className="text-xs text-gray-400 leading-relaxed flex gap-2">
								...
								<span>
									本活動票券一經售出，退換票請洽主辦單位。請於活動開始前 30
									分鐘入場。
								</span>
							</p>
						</div>
					...
					<div className="bg-white/20 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
							{relatedEvents.map(evt => (
								<div
									key={evt.id}
									className="h-full transform hover:-translate-y-1 transition-transform duration-300 border border-white/10 bg-white/5 rounded-xl overflow-hidden shadow-lg"
								>
									<HomeEventCard
										event={evt}
										isFavorited={false}
