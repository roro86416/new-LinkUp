	...
	return (
		<div className="min-h-screen bg-gray-50 flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
			<div className="max-w-md w-full space-y-8">
				...
				{isCameraMode ? (
					...
					<form className="mt-8 space-y-6" onSubmit={handleManualSubmit}>
						...
						<button
							type="submit"
							disabled={isLoading}
							className="group relative w-full flex justify-center py-3 px-4 border border-transparent text-lg font-medium rounded-md text-white bg-orange-600 hover:bg-orange-700 disabled:bg-gray-400 transition-colors"
						>
							{isLoading ? '驗證中...' : '送出驗證'}
						</button>
					</form>
				...
				{result && (
					<div
						className={`mt-6 p-6 rounded-xl border-2 shadow-lg transition-all duration-300 ${result.status === 'success' ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}
					>
						...
						<h3
							className={`text-center text-2xl font-bold mb-2 ${result.status === 'success' ? 'text-green-800' : 'text-red-800'}`}
						>
							{result.message}
						</h3>
						...
						{result.status === 'success' && result.data && (
							<div className="mt-4 space-y-2 text-center text-gray-700 border-t border-green-200 pt-4">
								<p>
									<span className="font-bold">活動：</span>{' '}
								...
								<p>
									...
									{new Date(result.data.scanTime).toLocaleString()}
								</p>
							</div>
						)}
					</div>
