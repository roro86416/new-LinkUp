...
export default function OrderTicketPage() {
	...
	const handleQuickValidate = async (qrCodeData: string, ticketId: number) => {
		try {
			// 呼叫後端 Check-in API
			await apiClient.post('/api/v1/check-in/verify', {
				qr_code: qrCodeData,
			});

			toast.success('🎉 驗票成功！');

			// 直接更新本地 state，讓畫面瞬間變換狀態
			setOrderData(prev => {
				if (!prev) return prev;
				const updatedItems = prev.items.map(item => {
					if (item.ticket && item.ticket.id === ticketId) {
						return { ...item, ticket: { ...item.ticket, status: 'used' } };
					}
					return item;
				});
				return { ...prev, items: updatedItems as OrderItemData[] };
			});
		} catch (error) {
			console.error('驗票失敗:', error);
			toast.error('驗票失敗，票券可能已失效');
		}
	};
	...
	if (isLoading) {
		return (
			<div className="min-h-screen flex justify-center items-center bg-[#0C2838] text-white">
				載入票券中...
			</div>
		);
	}
	...
	if (!orderData) {
		return (
			<div className="min-h-screen flex flex-col justify-center items-center bg-[#0C2838] text-white gap-4">
				...
				<button
					onClick={() => router.back()}
					className="text-[#EF9D11] hover:underline"
				>
					返回上一頁
				</button>
			</div>
		...
		<div className="fixed inset-0 z-[100] bg-[#0C2838] overflow-hidden flex flex-col">
			...
			<div className="relative z-10 flex items-center justify-between px-6 py-5 backdrop-blur-md bg-black/20 border-b border-white/10">
				...
				<div className="text-right">
					...
					<p className="text-sm font-mono text-[#EF9D11] font-bold">
				...
				<div className="w-full max-w-md h-full max-h-[80vh] overflow-y-auto custom-scrollbar pr-2 space-y-6">
					...
					<div className="text-center space-y-2 mb-6">
						...
						<div className="flex justify-center gap-4 text-sm text-gray-300/80">
							<span className="flex items-center gap-1">
								<CalendarIcon className="w-4 h-4 text-[#EF9D11]" />{' '}
								{eventDate.toLocaleDateString()}
							</span>
							<span className="flex items-center gap-1">
								<MapPinIcon className="w-4 h-4 text-[#EF9D11]" />{' '}
							...
							>
								...
								<div className="bg-white rounded-3xl overflow-hidden shadow-[0_0_40px_rgba(239,157,17,0.15)] transform transition-all hover:scale-[1.02] duration-300 relative">
									...
									<div className="bg-white p-8 flex flex-col items-center justify-center border-b-2 border-dashed border-gray-200 relative">
										...
										<p className="text-gray-400 text-[10px] font-mono tracking-[0.2em] uppercase">
											Scan to Enter
										</p>

										{/* 票券狀態標籤 */}
										<div
											className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-bold border shadow-sm
                                    ${
																			ticket.status === 'valid'
                                ...
                                `}
										>
											{ticket.status === 'valid' ? '可使用' : '已使用 / 已失效'}
										</div>
									</div>
									...
									<div className="bg-gray-50/80 p-6 pt-8 backdrop-blur-sm">
										...
										{ticket.status === 'valid' && (
											<button
												onClick={() =>
													handleQuickValidate(ticket.qr_code_data, ticket.id)
												}
												className="mt-5 w-full flex items-center justify-center gap-2 px-5 py-3 border-2 border-dashed border-[#EF9D11] text-[#EF9D11] bg-orange-50/50 rounded-xl text-sm font-bold hover:bg-[#EF9D11] hover:text-white transition-all duration-300 active:scale-95 shadow-sm"
											>
												🚀 [展示用] 模擬掃碼驗票
											</button>
										)}
					...
					<div className="text-center py-4 opacity-50">
						<img
							...
							alt="LinkUp"
							className="h-6 mx-auto invert brightness-0 opacity-50"
						/>
					</div>
				</div>
			</div>
