...
function SelectTicketContent() {
	...
	const handleSubmit = () => {
		...
		const selectedTicket = eventDetails?.ticketTypes.find(
				...
				<div className="flex-1 text-center sm:text-left">
					...
					<div className="flex flex-col sm:flex-row gap-3 text-sm text-gray-500 justify-center sm:justify-start">
						...
						{eventDetails.location_name && (
							<div className="flex items-center gap-1">
								<MapPinIcon className="w-4 h-4" /> {eventDetails.location_name}
							</div>
						)}
					</div>
					<div className="mt-4 flex items-center justify-center sm:justify-start gap-2">
						<div className="w-6 h-6 rounded-full bg-gray-200 overflow-hidden">
							{eventDetails.organizer.avatar ? (
								<img
				...
				<div className="space-y-4">
                  ...
                  ${
                      ...
                      ${selectedTicketId === ticket.id ? 'border-[#EF9D11] bg-[#EF9D11]' : 'border-gray-300 group-hover:border-orange-300'}`}
									...
									<div>
										<h3
											className={`font-bold text-lg ${selectedTicketId === ticket.id ? 'text-[#EF9D11]' : 'text-gray-800'}`}
										>
											{ticket.name}
										</h3>
										<p className="text-sm text-gray-500 mt-1">
											售票截止：
											{new Date(ticket.sale_end_time).toLocaleDateString()}
										</p>
									</div>
								</div>
								<span
									className={`text-xl font-bold ${selectedTicketId === ticket.id ? 'text-[#EF9D11]' : 'text-gray-900'}`}
								>
									NT$ {ticket.price}
								</span>
							</div>
...
export default function SelectTicketPage() {
	return (
		<Suspense fallback={<div>載入票券資訊中...</div>}>
			<SelectTicketContent />
		</Suspense>
	);
}
