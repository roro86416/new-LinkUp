...
export default function CheckoutInfoPage() {
	...
	useEffect(() => {
		...
		try {
			setCheckoutData(JSON.parse(dataString));
		} catch {
			toast.error('資料錯誤');
		} finally {
			setIsLoading(false);
	...
	const summary = useMemo(() => {
		...
		addons.forEach((qty, vid) => {
			...
			if (item)
				a += (item.product.base_price + item.variant.price_offset) * qty;
		});
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const payload = {
				...
				items: itemsPayload,
				attendees: [
					{ name: billingName, email: billingEmail, phone: billingPhone },
				], // 目前假設參加者同訂購者
			};
					...
					{checkoutData.availableProducts.length > 0 && (
						<section className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
							<h2 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
								<ShoppingBagIcon className="w-6 h-6 text-[#EF9D11]" /> 加購周邊
							</h2>
							<div className="grid gap-6">
								{checkoutData.availableProducts.map(product => (
									<div
										key={product.id}
										className="flex flex-col sm:flex-row gap-6 p-4 rounded-xl border border-gray-100 hover:border-orange-200 transition-colors bg-gray-50/50"
									>
												...
												{product.variants.map(v => {
													...
													const vName =
														v.option1_value === 'Default'
															? '標準款'
															: [v.option1_value, v.option2_value]
																	.filter(Boolean)
																	.join('/');
													return (
														<div
														...
														>
															...
															<div className="flex items-center gap-4">
																<span className="text-[#EF9D11] font-bold text-sm">
																	NT$ {price}
																</span>
																<div className="flex items-center gap-3 bg-gray-50 rounded-lg p-1 border border-gray-100">
																	<button
																		onClick={() =>
																			updateAddon(v.id, -1, v.stock_quantity)
																		}
																		disabled={qty === 0}
																		className={`p-1 rounded-md transition-colors ${qty === 0 ? 'text-gray-300' : 'text-gray-500 hover:bg-white hover:shadow-sm'}`}
																	>
																		<MinusIcon className="w-3 h-3" />
																	</button>
																	<span className="text-sm font-bold w-4 text-center text-gray-700">
																		{qty}
																	</span>
																	<button
																		onClick={() =>
																		...
																		className="p-1 rounded-md text-[#EF9D11] hover:bg-white hover:shadow-sm"
																	>
								...
								<div className="flex justify-between items-start">
									<div>
										<p className="font-bold text-gray-900 text-lg">
											{checkoutData.selectedTicket.name}
										</p>
								...
								<div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
									...
									<div className="space-y-4">
										{Array.from(addons.entries()).map(([vid, qty]) => {
											...
											if (!item) return null;
											const vName =
												item.variant.option1_value === 'Default'
													? ''
						...
						<div className="border-t border-dashed border-gray-200 pt-6">
							<div className="flex justify-between items-end">
								...
								<div className="text-right">
									<span className="block text-3xl font-bold text-[#EF9D11]">
										NT$ {summary.total}
									</span>
