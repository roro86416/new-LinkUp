...
export async function getEventDetail(id: number): Promise<EventDetailData> {
	try {
		const response = await apiClient.get<ApiResponse<EventDetailData>>(
			`/api/v1/events/${id}`,
		);
...
export async function getEvents(
	...
	try {
		const response =
			await apiClient.get<ApiResponse<EventCardData[]>>(endpoint);
		return response?.data || [];
	} catch (error) {
...
export async function checkCanReview(eventId: number): Promise<boolean> {
	try {
		...
		const response = await apiClient.get<any>(
			`/api/v1/events/${eventId}/can-review`,
		);
