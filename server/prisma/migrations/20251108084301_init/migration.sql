...
CREATE TABLE `PostReview` (
    ...
    INDEX `PostReview_post_id_idx`(`post_id`),
    INDEX `PostReview_user_id_idx`(`user_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `PostReview` ADD CONSTRAINT `PostReview_post_id_fkey` FOREIGN KEY (`post_id`) REFERENCES `user_posts`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `PostReview` ADD CONSTRAINT `PostReview_user_id_fkey` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
