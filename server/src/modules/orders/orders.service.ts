...
export const findOrdersByUserService = async (userId: string) => {
	...
	return orders.map(order => {
		...
		const eventData = order.items.find(i => i.ticketType?.event)?.ticketType
			?.event;
...
export const createOrderService = async (
	...
	const result = await prisma.$transaction(
		async tx => {
			...
			for (const item of items) {
				...
				if (item.item_type === ItemType.products) {
					...
					if (!item.product_variant_id)
						throw new Error('商品項目缺少 product_variant_id');
					...
					if (availableQuantity < quantity) {
						throw new Error(
							`商品 ${itemName} 庫存不足。剩餘: ${availableQuantity}`,
						);
					}
					...
					if (!item.ticket_type_id)
						throw new Error('票券項目缺少 ticket_type_id');
					...
					if (availableQuantity < quantity) {
						throw new Error(
							`活動票券 ${itemName} 數量不足。剩餘: ${availableQuantity}`,
						);
					}
			...
			// 驗證持票人數量是否與購買的票券總數相符
			const totalTicketsCount = ticketItemsInOrder.reduce(
				(sum, item) => sum + item.quantity,
				0,
			);
			if (totalTicketsCount !== attendees.length) {
				throw new Error(
					`持票人資料數量 (${attendees.length}) 與訂單中的票券總數 (${totalTicketsCount}) 不符。`,
				);
			}
			...
			if (totalAmount.toNumber() !== billingInfo.total_amount) {
				console.warn(
					`金額不符警告：後端計算 ${totalAmount}, 前端傳送 ${billingInfo.total_amount}`,
				);
			...
			for (const orderItem of newOrder.items) {
				if (
					orderItem.item_type === ItemType.ticket_types &&
